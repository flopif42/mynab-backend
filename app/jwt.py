...
class JwtManager:
    ...
    @staticmethod
    def get_instance():
        if JwtManager._instance is None:
            JwtManager._instance = JwtManager()
    ...
    def generate_access_token(id_user: int):
        token_exp_time = int(time.time()) + JwtManager._config['AccessToken']['LifespanSeconds']
        payload = {
            'algorithm': JwtManager._config['Algorithm'],
            'expirationTime': token_exp_time,
            'idUser': id_user
        }
        try:
            private_key_fd = open(JwtManager._config['PrivateKeyFile'])
            token = jwt.encode(payload, private_key_fd.read(), algorithm=JwtManager._config['Algorithm']).decode(JwtManager._config['Encoding'])
            response = make_response()
            response.set_cookie(JwtManager._config['AccessToken']['CookieName'], value=token, **JwtManager._config['CookieSettings'])
    ...
    def get_payload(request):
        try:
            token = request.cookies.get(JwtManager._config['AccessToken']['CookieName'])
            encoded_bytes = token.encode(encoding=JwtManager._config['Encoding'])
            public_key_fd = open(JwtManager._config['PublicKeyFile'])
            payload = jwt.decode(encoded_bytes, public_key_fd.read(), algorithms=[JwtManager._config['Algorithm']])
            return payload
