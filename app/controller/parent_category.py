...
from app.exceptions import InvalidParametersError, ParentCategoryNotFoundError, ParentCategoryNotEmptyError
...
def create(id_user, request):
    ...
    if len(parent_category_name) > 50:
        raise InvalidParametersError("Parent category name can't be more than 50 characters.")
    query = '''
...
def delete(id_user, request_params):
    try:
        ...
        if not is_deletable(id_parent, id_user):
            raise ParentCategoryNotEmptyError
        if id_parent == 0:
